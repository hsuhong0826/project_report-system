...
# from ftplib import FTP  # [NAS 已停用]
import mss
import mss.tools
...
# [單號已停用] generate_new_report_id 函式原用於產生報修單號，因無 NAS 功能已無用途
...
def submit_report():
    ...
    # [單號已停用] 無 NAS 功能，單號不再使用
    # report_id = generate_new_report_id()
    # timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # [NAS 已停用] record 字典原用於 FTP 上傳，目前不使用
    # record = {
    #     "ID": report_id,
    #     "姓名": name,
    #     "工號": staff_id,
