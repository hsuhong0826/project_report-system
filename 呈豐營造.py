...
CONFIG = {
    "name": "呈豐營造",  # 公司名稱
    "prefix": "F",  # 報修單 ID 前綴代碼
    "system_name": "呈豐營造-資訊報修系統",  # 視窗標題
    "password": "97400798",  # 若需要密碼驗證請填寫，否則留空
    "record_folder": r"C:\\贊耀報修紀錄_呈豐營造",  # 本地存檔路徑
...
def reset_fields():
    ...
    site_var.set("總公司")
    site_other_entry.delete(0, tk.END)
    site_other_entry.grid_remove()
...
def submit_report():
    ...
    try:
        ...
        site = clean_text(site_var.get())

        # 如果選擇「其他」，需要組合其他位置
        if site == "其他":
            site_other = clean_text(site_other_entry.get())
            if not site_other or site_other == "請輸入工地名稱":
                messagebox.showerror("錯誤", "請填寫其他工地名稱")
                return
            site = f"其他({site_other})"
        ...
        # 工地名稱為必填欄位
        if not site or site == "未填寫" or site == "":
            messagebox.showerror("錯誤", "工地名稱為必填欄位")
            return
...
def main():
    ...
    global site_var, site_other_entry

    root = tk.Tk()
    root.title(CONFIG["system_name"])

    # 設定 Combobox 下拉清單的字體大小
    root.option_add("*TCombobox*Listbox*Font", ("標楷體", 12))
    ...
    tk.Label(root, text="工地名稱：", font=("標楷體", 12)).grid(
    ...
    site_combobox.grid(row=2, column=1, padx=(10, 50), pady=5, sticky=tk.W)
    site_combobox.current(0)

    # 「其他」工地名稱輸入框（預設隱藏）
    site_other_entry = tk.Entry(root, width=26, font=("標楷體", 12), fg="grey")
    site_other_entry.insert(0, "請輸入工地名稱")
    site_other_entry.grid(row=2, column=1, padx=(10, 50), pady=5, sticky=tk.E)
    site_other_entry.grid_remove()

    def on_site_other_focus_in(event):
        if site_other_entry.get() == "請輸入工地名稱":
            site_other_entry.delete(0, tk.END)
            site_other_entry.config(fg="black")

    def on_site_other_focus_out(event):
        if site_other_entry.get() == "":
            site_other_entry.insert(0, "請輸入工地名稱")
            site_other_entry.config(fg="grey")

    site_other_entry.bind("<FocusIn>", on_site_other_focus_in)
    site_other_entry.bind("<FocusOut>", on_site_other_focus_out)

    def on_site_change(event):
        if site_var.get() == "其他":
            site_combobox.config(width=10)
            site_other_entry.grid()
        else:
            site_combobox.config(width=38)
            site_other_entry.grid_remove()

    site_combobox.bind("<<ComboboxSelected>>", on_site_change)
