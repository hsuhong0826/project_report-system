...
贊耀資訊"""
    ...
    try:
        with smtplib.SMTP(CONFIG["smtp_server"], CONFIG["smtp_port"], local_hostname="localhost") as server:
            server.starttls()
...
def engineer_ui_nas():
    ...
    def submit_solution():
        ...
        try:
            ...
            if not index.empty:
                ...
                df_main.at[index[0], "處理時間"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        try:
            send_email(matched_row["信箱"], matched_row["工號"], matched_row["問題描述"], solution)
        except Exception as e:
    ...
    tk.Label(window, text="工程師報修處理系統(揚發)", font=("Arial", 16, "bold")).pack(pady=10)
    ...
    tk.Button(window, text="搜尋", command=search_report, font=("Arial", 12)).pack(pady=5)

    frame_info = tk.Frame(window)
    frame_info.pack(pady=10)
    tk.Label(frame_info, text="工號：", font=("Arial", 12)).grid(row=0, column=0, sticky="e")
    label_staff = tk.Label(frame_info, text="", font=("Arial", 12))
    label_staff.grid(row=0, column=1, sticky="w")
    tk.Label(frame_info, text="信箱：", font=("Arial", 12)).grid(row=1, column=0, sticky="e")
    ...
    tk.Button(window, text="送出處理結果", font=("Arial", 12, "bold"), command=submit_solution).pack(pady=10)

    footer_label = tk.Label(window, text="Copyright by ZY-Info V1.1", font=("Arial", 12))
